...
-- พร้อมคำอธิบาย Logic การเขียนเป็นภาษาไทย
-- ============================================================
...
-- Logic: Correlated Subquery แต่ละตัวรับ tutor_id / user_id จาก outer query
--        monthly_income ดึงจาก transaction_logs ที่ transaction_type = 'tutor_earnings'
--        เพื่อให้ตรงกับเงินที่ได้รับจริงใน wallet ไม่ใช่ budget จากโพสต์
...
SELECT
    ...
    (SELECT COALESCE(SUM(tl.amount), 0)
     ...
     WHERE w.user_id = 1                          -- ใช้ user_id แทน tutor_id
